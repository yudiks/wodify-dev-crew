...
REPO_CONVENTIONS = (
    ...
    "If you need to change prisma/schema.prisma: `prisma migrate dev` against this "
    "project's local Postgres (a `prisma dev` server) is unreliable — it can hang or "
    "fail with spurious connection errors. The proven workaround: hand-write the "
    "migration SQL yourself as a new prisma/migrations/<N>_<name>/migration.sql file "
    "(follow the style of existing migrations in that directory), then apply it "
    "directly with a short Node script using the `pg` package and process.env."
    "DATABASE_URL (see how earlier migrations were applied if you want a reference), "
    "then run `npx prisma generate` to refresh the client. Don't waste time re-diagnosing "
    "transient DB connection errors during `npm test` — if a single retry of the same "
    "command succeeds, that confirms it was transient, not a real bug; move on.\n\n"
    "Use the Run Claude Code tool sparingly and purposefully: prefer one call that "
    "states the end goal and lets Claude Code explore the repo and implement it "
    "itself, over several calls that first ask Claude Code to dump file contents "
    "back to you. Claude Code can read its own codebase fine without you "
    "pre-fetching files for it — and large dumped-back file contents eat into your "
    "own context for no benefit. Only ask it to show you something specific (e.g. "
    "a command's output, a short snippet) when you genuinely need to inspect the "
